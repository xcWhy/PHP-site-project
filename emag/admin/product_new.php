...
<div class="columns">
    ...
    <form action="product_new.php" method="post" id="form1"  enctype="multipart/form-data">
            ...
            <select name="category_id">
                <?php
                    ...
                    while ($row = mysqli_fetch_array($result))  {                  
                        echo "<option " . (isset($category_id) && $category_id == $row['id'] ? 'selected' : '') . " value=\"{$row['id']}\">{$row['name']}</option>";
                    }
                ?>
            </select>
            </div>
        ...
        <div class="control">
            <textarea name="description" class="textarea"><?php echo isset($description) ? $description : '' ?></textarea>
        </div>
        </div>
        ...
        <div class="control">
            ...
            <input name="is_promo" type="checkbox" <?php echo isset($is_promo) && $is_promo ? 'checked' : '' ?>>
            Promotional
            </label>
        </div>
        </div>
        ...
        <div class="control">
            <label class="radio">
            <input type="radio" name="availability" value="out_of_stock" <?php echo isset($availability) && ($availability == 'out_of_stock') ? 'checked' : '' ?>>
            Out of stock
            </label>
            <label class="radio">
            <input type="radio" name="availability" value="available" <?php echo !isset($availability) || (isset($availability) && ($availability == 'available')) ? 'checked' : '' ?>>
            Available
            </label>
            <label class="radio">
            <input type="radio" name="availability" value="expected" <?php echo isset($availability) && ($availability == 'expected') ? 'checked' : '' ?>>
            Expected
            </label>
        </div>
        </div>
